...
@_R().blueprint.route('/api/intrag/ndf')
def api_intrag_ndf():
    if not _R().session.get('authenticated'):
        return _R().jsonify({'success': False, 'message': 'Not authenticated'}), 401
    date_str  = _R().request.args.get('date', '').strip()       # YYYY-MM-DD (single day)
    date_from = _R().request.args.get('date_from', '').strip()  # YYYY-MM-DD (range start)
    date_to   = _R().request.args.get('date_to', '').strip()    # YYYY-MM-DD (range end)
    entries = []
    if date_from or date_to:
        ...
        try:
            ref = _R().datetime.strptime(date_str, '%Y-%m-%d')
            fname = ref.strftime('%Y%m%d') + '_intrag_ndf.json'
            fp = _R().os.path.join(engine.INTRAG_NDF_CACHE_DIR, ref.strftime('%Y'), ref.strftime('%m'), fname)
        ...
        for fp, _fname, mtime, size in _R()._day_files(engine.INTRAG_NDF_CACHE_DIR, '_intrag_ndf.json'):
            entries.extend(_R()._day_json(fp, mtime, size))
    return _R().jsonify({'success': True, 'entries': entries})

@_R().blueprint.route('/api/intrag/option')
def api_intrag_option():
    if not _R().session.get('authenticated'):
        return _R().jsonify({'success': False, 'message': 'Not authenticated'}), 401
    date_str  = _R().request.args.get('date', '').strip()
    date_from = _R().request.args.get('date_from', '').strip()
    date_to   = _R().request.args.get('date_to', '').strip()
    ...
    if date_from or date_to:
        ...
        try:
            ref = _R().datetime.strptime(date_str, '%Y-%m-%d')
            fp = _R().os.path.join(engine.INTRAG_OPT_CACHE_DIR, ref.strftime('%Y'), ref.strftime('%m'),
...
@_R().blueprint.route('/api/intrag/option/send-file', methods=['POST'])
def api_intrag_option_send_file():
    ...
    if not isinstance(items, list) or not items:
        ...
        if not isinstance(rows, list) or not rows:
            return _R().jsonify({'success': False, 'message': 'No rows provided'}), 400
    ...
    for it in items:
        ...
        ref = _R()._parse_date_any(td_raw) or _R().datetime.now()
        groups.setdefault(ref.strftime('%Y%m%d'), {'ref': ref, 'rows': []})['rows'].append(cells)
        if it.get('deal_id'):
            sent_ids.append((it['deal_id'], td_raw))

    if not groups:
        return _R().jsonify({'success': False, 'message': 'No valid rows provided'}), 400
    ...
    try:
        with _R()._cache_lock:
            for key, grp in groups.items():
                ...
                dir_path = _R().os.path.join(engine.INTRAG_NDF_SEND_DIR, ref.strftime('%Y'), month_folder, ref.strftime('%d'))
                _R().os.makedirs(dir_path, exist_ok=True)
                base = 'Intrag-Option-' + key
                candidate = base + '.txt'
                n = 0
                while _R().os.path.exists(_R().os.path.join(dir_path, candidate)):
                    n += 1
                    candidate = base + ' (' + str(n) + ').txt'
                file_path = _R().os.path.join(dir_path, candidate)
        ...
        return _R().jsonify({'success': False, 'message': 'File generation failed: ' + str(exc)}), 500
    ...
    return _R().jsonify({'success': True, 'files': written, 'count': len(items)})

@_R().blueprint.route('/api/intrag/option/edit', methods=['POST'])
def api_intrag_option_edit():
    ...
    if not deal_id:
        return _R().jsonify({'success': False, 'message': 'Missing deal_id'}), 400
    with _R()._cache_lock:
        fp, entries, idx = engine._find_intrag_opt_entry(deal_id, trade_date)
        if idx is None:
            return _R().jsonify({'success': False, 'message': 'Entry not found'}), 404
        if isinstance(fields, dict):
            for k, v in fields.items():
                if k in entries[idx] and k not in ('_deal', '_client', 'status', 'maker', 'checker'):
                    entries[idx][k] = v
        entries[idx]['status']  = 'Pending'
        entries[idx]['maker']   = _R().session.get('user_sid', '')
        entries[idx]['checker'] = ''
        _R()._atomic_write_json(fp, entries)
    _R()._create_notification(_R().session.get('user_sid', ''), _R().session.get('user_name', ''),
                         'Deal Updated', 'Intrag Option', deal_id)
    return _R().jsonify({'success': True, 'status': 'Pending'})

@_R().blueprint.route('/api/intrag/option/approve', methods=['POST'])
def api_intrag_option_approve():
    ...
    if not deal_id:
        return _R().jsonify({'success': False, 'message': 'Missing deal_id'}), 400
    user_sid = _R().session.get('user_sid', '')
    with _R()._cache_lock:
        fp, entries, idx = engine._find_intrag_opt_entry(deal_id, trade_date)
        if idx is None:
            return _R().jsonify({'success': False, 'message': 'Entry not found'}), 404
        if (entries[idx].get('status') or '') != 'Pending':
            return _R().jsonify({'success': False, 'message': 'Only Pending entries can be approved.'}), 400
    ...
    return _R().jsonify({'success': True, 'status': 'Approved'})

@_R().blueprint.route('/api/intrag/ndf/send-file', methods=['POST'])
...
@_R().blueprint.route('/api/intrag/ndf/edit', methods=['POST'])
...
@_R().blueprint.route('/api/intrag/ndf/approve', methods=['POST'])
...
@_R().blueprint.route('/api/intrag/ndf/mapping-intrag-id', methods=['POST'])
def api_intrag_ndf_mapping_intrag_id():
    if not _R().session.get('authenticated'):
        return _R().jsonify({'ok': False, 'error': 'Not authenticated'}), 401
    deals = (_R().request.get_json(silent=True) or {}).get('deals', [])
    results, err = engine._intrag_run_mapping(deals, 1, 'NDF - TERMO MERCADORIA', 2, engine._find_intrag_ndf_entry)
    if results is None:
        return _R().jsonify({'ok': False, 'error': err}), 400
    return _R().jsonify({'ok': True, 'results': results})

@_R().blueprint.route('/api/intrag/option/mapping-intrag-id', methods=['POST'])
...
@_R().blueprint.route('/api/intrag/swap')
def api_intrag_swap():
    if not _R().session.get('authenticated'):
        return _R().jsonify({'success': False, 'message': 'Not authenticated'}), 401
    date_str  = _R().request.args.get('date', '').strip()       # YYYY-MM-DD (single day)
    ...
    if date_from or date_to:
        ...
        try:
            ref = _R().datetime.strptime(date_str, '%Y-%m-%d')
            fp = _R().os.path.join(engine.INTRAG_SWAP_CACHE_DIR, ref.strftime('%Y'), ref.strftime('%m'),
                              ref.strftime('%Y%m%d') + suffix)
            if _R().os.path.isfile(fp):
                with open(fp, 'r', encoding='utf-8') as fh:
                    entries = _R().json.load(fh)
...
@_R().blueprint.route('/api/intrag/swap/send-file', methods=['POST'])
...
@_R().blueprint.route('/api/intrag/swap/edit', methods=['POST'])
...
@_R().blueprint.route('/api/intrag/swap/approve', methods=['POST'])
...
@_R().blueprint.route('/api/intrag/swap/mapping-intrag-id', methods=['POST'])
