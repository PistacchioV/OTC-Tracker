...
def send(kind, rows, to_list, cc_list, ref):
    ...
    try:
        with R._app_context():
            html = render_template('pages/email-template-manual-deals-ea.html',
                                   ...
                                   routine=domain.LABEL.get(kind, kind),
                                   rows=rows, current_year=datetime.now().year)
            msg = MIMEMultipart('related')
            msg['Subject'] = domain.subject(kind, ref)
        ...
        R.log.error('[manual-deals-ea] envio falhou:\n%s', traceback.format_exc())
