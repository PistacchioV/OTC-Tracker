...
def api_generic_nd_send_conecta(product):
    ...
    try:
        ...
        for deal, bucket in made_by_deal:
            ...
            counts[fname] += 1
        # Quebra visão MGT x Cliente (só o VANILLA pediu): a mesa booka contra
        # a MGT (04880.00-6 x 73760.20-5, template do File Interpreter já
        ...
        # o espelho é sintetizado como o do Lawton; as linhas de verificação
        # (tipo 2) acompanham, senão o registro asiático do lado do banco iria
        # para a B3 sem as datas.
        if product == 'vanilla':
            for deal, bucket in made_by_deal:
                if bucket != 'MGT':
                    continue
