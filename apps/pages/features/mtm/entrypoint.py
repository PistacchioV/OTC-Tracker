...
def api_mtm_data():
    ...
    with _R()._cache_lock:
        path, data = persistence._mtm_load(request.args.get('date'))
        if not data:
        ...
        if path and queries._mtm_normalize_zeros(data):
            try:
...
def api_mtm_latest():
    if not session.get('authenticated'):
        return jsonify({'success': False, 'error': 'Not authenticated'}), 401
    return jsonify({'success': True, 'date': persistence._mtm_latest_ymd()})
...
def api_mtm_import_folder():
    ...
    try:
        persistence._mtm_save(persistence._mtm_path_for(ymd), result)
    except Exception:
        _R().log.error('[mtm] save failed:\n%s', traceback.format_exc())
        return jsonify({'success': False, 'error': 'Failed to save the imported data.'}), 500

    swap_n = sum(result['counts'].get(k, 0) for k in domain._MTM_SWAP_BOOKS)
...
def api_mtm_row_comment():
    ...
    with _R()._cache_lock:                        # ler → alterar → gravar
        path, data = persistence._mtm_load(p.get('date'))
        if not data:
...
def api_mtm_process():
    ...
    with _R()._cache_lock:
        ...
        if domain._mtm_is_cem_value_name(f.filename):
            ...
            em, cm, z, miss = mappers._mtm_apply_edg_values(data, rows)  # JP* → COE, else EDG; + Missing MtM
            data['diagnostics'] = dict(data.get('diagnostics') or {},
                                       edg_value_file=f.filename, edg_matched=em, edg_coe_matched=cm,
                                       edg_zeros=z, edg_missing=miss)
        elif domain._mtm_is_coe_name(f.filename):
            coe_rows, coe_ref = queries._mtm_build_coe(rows)
            for i, rw in enumerate(coe_rows):
                rw.extend(['New', '', ''])
                rw.append('COE-{}'.format(i))
            data['tables']['COE'] = coe_rows
            data['coe_ref_date'] = coe_ref
        elif domain._mtm_is_swap_name(f.filename):
            buckets, ref_date, _kept, _matched = queries._mtm_build_swap(rows)
            for lob in domain._MTM_SWAP_BOOKS:
                rws = buckets.get(lob, [])
        ...
        try:
            persistence._mtm_save(persistence._mtm_path_for(ymd), data)
        except Exception:
...
def api_mtm_send_batch():
    ...
    try:
        files = commands._mtm_generate_coe(rows, ymd) if lob == 'COE' else commands._mtm_generate_book(lob, rows, ymd)
    except ValueError:
    ...
    written = commands._mtm_write_gen_files(files, ymd)
    _R()._create_notification(session.get('user_sid', ''), session.get('user_name', ''),
                         'MTM Sent', 'MtM',
                         '{} · {} file(s)'.format(lob, len(files)) + _R()._nd_token(ymd))
    return jsonify({'success': True, 'files': commands._mtm_gen_preview(files), 'written': len(written)})
...
def api_mtm_row_preview():
    ...
    if row is None:
        return jsonify({'success': False, 'error': 'Row not found.'}), 404
    if str(row[-4]) == domain._MTM_STATUS_MISSING:
        return jsonify({'success': False, 'error': 'missing_mtm'}), 400
    try:
        files = commands._mtm_generate_coe([row], ymd) if lob == 'COE' else commands._mtm_generate_book(lob, [row], ymd)
    except ValueError:
    ...
    return jsonify({'success': True, 'files': commands._mtm_gen_preview(files)})
...
def api_mtm_validation():
    ...
    p = request.get_json(silent=True) or {}
    ymd = _R()._accrual_parse_date(p.get('date')) or datetime.now().strftime('%Y%m%d')
    path, data = persistence._mtm_load(datetime.strptime(ymd, '%Y%m%d').strftime('%Y-%m-%d'))
    if not data or not data.get('tables'):
        return jsonify({'success': False, 'error': 'No saved data for this date.'}), 404

    missing = domain._mtm_missing_rows(data, list(domain._MTM_VAL_BOOKS) + ['COE'])   # block if any row lacks a value
    if missing:
    ...
    files = {}
    try:
        for lob in domain._MTM_VAL_BOOKS:
            rows = tables.get(lob) or []
            if rows:
                files.update(commands._mtm_generate_book(lob, rows, ymd))
        coe_rows = tables.get('COE') or []
        if coe_rows:
            files.update(commands._mtm_generate_coe(coe_rows, ymd))
    except Exception:
    ...
    commands._mtm_write_gen_files(files, ymd)
        ...
        for lob_rows in (data.get('tables') or {}).values():
            for r in lob_rows or []:
                if r and len(r) >= 4:
                    r[-4], r[-2] = 'Sent', sid
        try:
            persistence._mtm_save(path, data)
        except Exception:
    ...
    try:
        ...
        _R().threading.Thread(target=mail._send_mtm_validation_email,
...
def api_mtm_end_process():
    ...
    checks, pending = domain._mtm_check_status_rows(data)
    if pending:
    ...
    try:
        html = render_template(
            ...
            folder=persistence._mtm_source_dir(ymd), current_year=datetime.now().year)
        logo_path = _R()._get_logo_path()
        _R().threading.Thread(target=mail._send_mtm_endprocess_email,
...
def api_mtm_recon():
    ...
    path, data = persistence._mtm_load(date_arg)
    if not data or not data.get('tables'):
        return jsonify({'success': False, 'error': 'No saved data for this date.'}), 404
    ...
    try:
        if f and f.filename:
            ...
            if not op:
                return jsonify({'success': False,
                                'error': 'ConsultaInformacoesAtualizMID file not found in {}'.format(persistence._mtm_source_dir(ymd))}), 400
    ...
    with _R()._cache_lock:
        ...
        if not data or not data.get('tables'):
            return jsonify({'success': False, 'error': 'No saved data for this date.'}), 404
        summary = commands._mtm_run_recon(data, rows)
        try:
            persistence._mtm_save(path, data)
        except Exception:
