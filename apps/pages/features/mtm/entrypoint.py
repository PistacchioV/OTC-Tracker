...
@_R().blueprint.route('/mtm-swap')
def mtm_swap():
    if not _R().session.get('authenticated'):
        return _R().redirect(_R().url_for('pages_blueprint.sign_in_page'))
    return _R().render_template('pages/mtm-swap.html', segment='mtm-swap',
                           today=_R()._br_now().strftime('%Y-%m-%d'))

@_R().blueprint.route('/api/mtm-swap/data')
def api_mtm_data():
    if not _R().session.get('authenticated'):
    ...
    with _R()._cache_lock:
        path, data = engine._mtm_load(_R().request.args.get('date'))
        if not data:
            return _R().jsonify({'success': True, 'empty': True})
    ...
    data['success'] = True
    return _R().jsonify(data)

@_R().blueprint.route('/api/mtm-swap/latest')
def api_mtm_latest():
    if not _R().session.get('authenticated'):
        return _R().jsonify({'success': False, 'error': 'Not authenticated'}), 401
    return _R().jsonify({'success': True, 'date': engine._mtm_latest_ymd()})

@_R().blueprint.route('/api/mtm-swap/mapping/add', methods=['POST'])
def api_mtm_mapping_add():
    ...
    if not (b3 and hyb and name):
        return _R().jsonify({'success': False, 'error': 'All three fields are required.'}), 400
    ...
    return _R().jsonify({'success': True, 'count': len(mapping),
                    'entry': {'b3_id': b3, 'hybrids_id': hyb, 'trade_name': name}})

@_R().blueprint.route('/api/mtm-swap/import-folder', methods=['POST'])
def api_mtm_import_folder():
    if not _R().session.get('authenticated'):
        return _R().jsonify({'success': False, 'error': 'Not authenticated'}), 401
    p   = _R().request.get_json(silent=True) or {}
    ymd = _R()._accrual_parse_date(p.get('date')) or _R().datetime.now().strftime('%Y%m%d')
    folder = engine._mtm_source_dir(ymd)
    if not _R().os.path.isdir(folder):
        return _R().jsonify({'success': False, 'error': 'Folder not found: {}'.format(folder)}), 400
    try:
        ...
        return _R().jsonify({'success': False, 'error': 'Failed to read the MTM files.'}), 500
    if not swap_fn and not coe_fn:
        return _R().jsonify({'success': False, 'error': 'No MTM files found in {}'.format(folder)}), 400

    result['date'] = _R().datetime.strptime(ymd, '%Y%m%d').strftime('%Y-%m-%d')
    try:
        engine._mtm_save(engine._mtm_path_for(ymd), result)
    except Exception:
        _R().log.error('[mtm] save failed:\n%s', _R().traceback.format_exc())
        return _R().jsonify({'success': False, 'error': 'Failed to save the imported data.'}), 500
    ...
    return _R().jsonify(result)

@_R().blueprint.route('/api/mtm-swap/row/comment', methods=['POST'])
def api_mtm_row_comment():
    ...
    with _R()._cache_lock:                        # ler → alterar → gravar
        ...
        if not data:
            return _R().jsonify({'success': False, 'error': 'No saved data for this date.'}), 404
...
@_R().blueprint.route('/api/mtm-swap/row/edit', methods=['POST'])
def api_mtm_row_edit():
    ...
    with _R()._cache_lock:                        # ler → alterar → gravar
        ...
        try:
            ...
            return _R().jsonify({'success': False, 'error': 'Save failed.'}), 500
    _R()._create_notification(sid, _R().session.get('user_name', ''), 'MTM Updated', 'MtM',
                         '{} · {}'.format(p.get('lob', ''), p.get('id', '')) + _R()._nd_token(p.get('date')))
    return _R().jsonify({'success': True, 'row': r})

@_R().blueprint.route('/api/mtm-swap/row/send', methods=['POST'])
def api_mtm_row_send():
    ...
    with _R()._cache_lock:
        ...
        if str(r[-3] or '') == sid:                          # maker == current user → blocked
            return _R().jsonify({'success': False, 'error': 'same_user'}), 403
        r[-4], r[-2] = 'Sent', sid                           # status, checker
        try:
            _R()._atomic_write_json(path, data)
        except Exception:
            return _R().jsonify({'success': False, 'error': 'Save failed.'}), 500
    _R()._create_notification(sid, _R().session.get('user_name', ''), 'MTM Sent', 'MtM',
                         '{} · {}'.format(p.get('lob', ''), p.get('id', '')) + _R()._nd_token(p.get('date')))
    return _R().jsonify({'success': True, 'row': r})

@_R().blueprint.route('/api/mtm-swap/row/delete', methods=['POST'])
def api_mtm_row_delete():
    ...
    with _R()._cache_lock:                        # ler → remover → gravar
        ...
        if rows is None:
            return _R().jsonify({'success': False, 'error': 'Book not found.'}), 404
    ...
    return _R().jsonify({'success': True, 'counts': data['counts']})

@_R().blueprint.route('/api/mtm-swap/rows/delete', methods=['POST'])
...
@_R().blueprint.route('/api/mtm-swap/process', methods=['POST'])
def api_mtm_process():
    ...
    if not f or not f.filename:
        return _R().jsonify({'success': False, 'error': 'No file provided.'}), 400
    ymd = _R()._accrual_parse_date(_R().request.form.get('date')) or _R().datetime.now().strftime('%Y%m%d')
    try:
        ...
        return _R().jsonify({'success': False, 'error': 'Failed to read the file.'}), 500
    ...
    with _R()._cache_lock:
        ...
        _, data = engine._mtm_load(_R().datetime.strptime(ymd, '%Y%m%d').strftime('%Y-%m-%d'))
        if not data:
            data = {'success': True, 'tables': {k: [] for k in engine._MTM_SWAP_BOOKS},
                    'counts': {}, 'ref_date': None, 'coe_ref_date': None,
                    'date': _R().datetime.strptime(ymd, '%Y%m%d').strftime('%Y-%m-%d')}
            data['tables']['COE'] = []
        ...
        try:
            ...
            return _R().jsonify({'success': False, 'error': 'Failed to save.'}), 500
    _R()._create_notification(_R().session.get('user_sid', ''), _R().session.get('user_name', ''),
                         'MTM Imported', 'MtM', f.filename + _R()._nd_token(ymd))
    return _R().jsonify(data)

@_R().blueprint.route('/api/mtm-swap/send-batch', methods=['POST'])
def api_mtm_send_batch():
    ...
    if not rows:
        return _R().jsonify({'success': False, 'error': 'No rows in this book to generate.'}), 400
    try:
        ...
        _R().log.error('[mtm] send-batch build failed:\n%s', _R().traceback.format_exc())
        return _R().jsonify({'success': False, 'error': 'Generation failed.'}), 500
    if not files:
        return _R().jsonify({'success': False, 'error': 'Nothing to generate for this book.'}), 400
    written = engine._mtm_write_gen_files(files, ymd)
    _R()._create_notification(_R().session.get('user_sid', ''), _R().session.get('user_name', ''),
                         'MTM Sent', 'MtM',
                         '{} · {} file(s)'.format(lob, len(files)) + _R()._nd_token(ymd))
    return _R().jsonify({'success': True, 'files': engine._mtm_gen_preview(files), 'written': len(written)})

@_R().blueprint.route('/api/mtm-swap/row/preview', methods=['POST'])
def api_mtm_row_preview():
    ...
    if str(row[-4]) == engine._MTM_STATUS_MISSING:
        return _R().jsonify({'success': False, 'error': 'missing_mtm'}), 400
    try:
        ...
        return _R().jsonify({'success': False, 'error': 'Generation failed.'}), 500
    if not files:
        return _R().jsonify({'success': False, 'error': 'Nothing to generate for this row.'}), 400
    return _R().jsonify({'success': True, 'files': engine._mtm_gen_preview(files)})

@_R().blueprint.route('/api/mtm-swap/validation', methods=['POST'])
def api_mtm_validation():
    ...
    p = _R().request.get_json(silent=True) or {}
    ymd = _R()._accrual_parse_date(p.get('date')) or _R().datetime.now().strftime('%Y%m%d')
    path, data = engine._mtm_load(_R().datetime.strptime(ymd, '%Y%m%d').strftime('%Y-%m-%d'))
    ...
    if missing:
        return _R().jsonify({'success': False, 'error': 'missing_accrual', 'missing': missing}), 400
    ...
    try:
        ...
        return _R().jsonify({'success': False, 'error': 'Failed to write the batch files.'}), 500
    if not files:
        return _R().jsonify({'success': False, 'error': 'No records to validate.'}), 400
    ...
    attach = [_R().os.path.join(_R().CONECTA_NEW_PATH, fn + '.txt')
    ...
    try:
        html = _R().render_template(
            ...
            attachment_names=[_R().os.path.basename(a) for a in attach],
    ...
    return _R().jsonify({'success': True, 'files': summary,
                    'attached': [_R().os.path.basename(a) for a in attach],
                    'total': total, 'mail': 'queued'})

@_R().blueprint.route('/api/mtm-swap/end-process', methods=['POST'])
def api_mtm_end_process():
    ...
    if pending:
        return _R().jsonify({'success': False, 'error': 'uncommented', 'pending': pending}), 400
    ...
    try:
        html = _R().render_template(
            ...
            folder=engine._mtm_source_dir(ymd), current_year=_R().datetime.now().year)
        logo_path = _R()._get_logo_path()
    ...
    return _R().jsonify({'success': True, 'checks': len(checks)})

@_R().blueprint.route('/api/mtm-swap/recon', methods=['POST'])
def api_mtm_recon():
    ...
    if not _R().session.get('authenticated'):
        return _R().jsonify({'success': False, 'error': 'Not authenticated'}), 401
    f = _R().request.files.get('file')
    date_arg = _R().request.form.get('date')
    path, data = engine._mtm_load(date_arg)
    if not data or not data.get('tables'):
        return _R().jsonify({'success': False, 'error': 'No saved data for this date.'}), 404

    ymd = _R()._accrual_parse_date(date_arg) or _R().datetime.now().strftime('%Y%m%d')
    try:
        if f and f.filename:
            rows = _R()._cc_read_rows(f.filename, f.read())
        else:
            ...
            with open(op, 'rb') as fh:
                rows = _R()._cc_read_rows(_R().os.path.basename(op), fh.read())
    except ValueError as e:
        return _R().jsonify({'success': False, 'error': str(e)}), 400
    except Exception:
        _R().log.error('[mtm] recon read failed:\n%s', _R().traceback.format_exc())
        return _R().jsonify({'success': False, 'error': 'Failed to read the recon file.'}), 500
    ...
    with _R()._cache_lock:
        ...
        try:
            ...
            return _R().jsonify({'success': False, 'error': 'Failed to save the recon result.'}), 500
