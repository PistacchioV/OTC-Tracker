...
(function () {
  ...
  var API = '/api/ndf-cockpit/data';
  var IMPORT_API = '/api/ndf-cockpit/import';
  var page = document.getElementById('ndfc-page');
  if (!page) return;
  ...
  function load(dateStr) {
    fetch(API + (dateStr ? ('?date=' + encodeURIComponent(dateStr)) : ''), { credentials: 'same-origin' })
      ...
      .then(function (d) {
        ...
        var w = d.widgets || {};
        setVal('ndfc-w-cpty', w.counterparties || 0); setVal('ndfc-w-notional', w.notional || '0.00');
        setVal('ndfc-w-settlement', w.settlement || '0.00'); setVal('ndfc-w-total', w.total || 0);
        setVal('ndfc-updated', d.updated ? (t('updated') + ' ' + d.updated) : '');
  ...
  function actionsHtml(id) {
    return '<div class="d-inline-flex gap-1">' +
      '<button class="btn btn-info btn-sm rounded-circle btn-row-edit" data-id="' + esc(id) + '" title="' + esc(t('edit')) + '"><i class="ti ti-edit"></i></button>' +
  ...
  function buildTable(columns, rows) {
    ...
    var titleRow =
      '<tr id="ndfc-head">' +
      '<th class="text-center" style="min-width:38px"><input type="checkbox" id="ndfcCheckAll" class="form-check-input"></th>' +
      '<th data-lang="ndfc-actions">Actions</th>' +
      '<th data-lang="ndfc-status">Status</th>' +
      columns.map(function (c) { return '<th>' + esc(c) + '</th>'; }).join('') + '</tr>';
    var filterRow =
      '<tr class="ndfc-th-filter">' +
      '<th></th><th></th>' +
      '<th><input type="text" class="form-control form-control-sm ndfc-col-filter" data-col="2" placeholder="Status" autocomplete="off"></th>' +
      columns.map(function (c, i) {
        return '<th><input type="text" class="form-control form-control-sm ndfc-col-filter" data-col="' +
          (i + 3) + '" placeholder="' + esc(c) + '" autocomplete="off"></th>';
      }).join('') + '</tr>';
    document.querySelector('#ndfc-table thead').innerHTML = titleRow + filterRow;
    ...
    jQuery('#ndfc-table thead').off('keyup.ndfccol change.ndfccol')
      .on('keyup.ndfccol change.ndfccol', '.ndfc-col-filter', function () {
        var col = +this.getAttribute('data-col');
        this.classList.toggle('ndfc-has-val', !!this.value);
        if (dt.column(col).search() !== this.value) dt.column(col).search(this.value).draw();
      });

    setVal('ndfc-count', rows.length);
    buildColumnsToggle();
    var sel = document.querySelector('.ndfc-page-len');   // sync "Show entries" to current length
  ...
  function buildColumnsToggle() {
    ...
    var items = dt.columns().header().toArray().slice(2).map(function (th, index) {
      ...
      return '<li class="px-3 py-1"><div class="form-check">' +
        '<input class="form-check-input toggle-vis" type="checkbox" data-column="' + (index + 2) + '" id="ndfcCol' + index + '" checked>' +
        '<label class="form-check-label fw-medium" for="ndfcCol' + index + '">' + esc(label) + '</label>' +
        '</div></li>';
    }).join('');
    wrap.innerHTML =
      '<div class="dropdown">' +
      ...
      '<ul class="dropdown-menu" id="ndfcColMenu" style="max-height:340px; overflow-y:auto;">' + items + '</ul></div>';
    document.getElementById('ndfcColMenu').addEventListener('change', function (e) {
  ...
  function wireClear() {
    var clr = document.getElementById('ndfcClearFilters');
    if (clr) clr.addEventListener('click', function () {
      document.querySelectorAll('#ndfc-table .ndfc-col-filter').forEach(function (i) { i.value = ''; i.classList.remove('ndfc-has-val'); });
      if (dt) { dt.columns().every(function () { this.search(''); }); dt.draw(); }
    });
  }

  function currentDate() {
    var inp = document.getElementById('ndfc-date');
    if (window.jQuery && jQuery('#ndfc-date').data && jQuery('#ndfc-date').data('daterangepicker')) {
      return jQuery('#ndfc-date').data('daterangepicker').startDate.format('YYYY-MM-DD');
    }
  ...
  function wireDatePicker(attempt) {
    ...
    if (window.jQuery && jQuery.fn.daterangepicker && window.moment) {
      ...
      jQuery('#ndfcDateWrap .ndfc-cal-btn').on('click', function () { $d.trigger('click'); });
      return;
    }
    attempt = attempt || 0;
  ...
  function wirePageLen() {
    var sel = document.querySelector('.ndfc-page-len');
  ...
  function openModal(editId, prefillCells) {
    EDIT_ID = editId || null;
    var title = document.getElementById('ndfcModalTitle');
    ...
    if (window.bootstrap) bootstrap.Modal.getOrCreateInstance(document.getElementById('ndfcAddModal')).show();
  }

  function afterMutation(titleKey) {
    if (window.bootstrap) {
      var inst = bootstrap.Modal.getInstance(document.getElementById('ndfcAddModal'));
      if (inst) inst.hide();
    }
    load(currentDate());
    if (window.fetchNotifications) window.fetchNotifications();
  ...
  function wireAddRow() {
    ...
    var save = document.getElementById('ndfcAddSave');
    if (save) save.addEventListener('click', function () {
      var cells = [];
      document.querySelectorAll('#ndfcAddFields .ndfc-add-fld').forEach(function (f) { cells[+f.getAttribute('data-i')] = f.value; });
      var url = EDIT_ID ? '/api/ndf-cockpit/row/edit' : '/api/ndf-cockpit/row/add';
  ...
  function wireActions() {
    jQuery('#ndfc-table').off('click.ndfcact')
      .on('click.ndfcact', '.btn-row-edit', function () {
      ...
      .on('click.ndfcact', '.btn-row-confirm', function () {
        var id = this.getAttribute('data-id');
        postJSON('/api/ndf-cockpit/row/confirm', { date: currentDate(), id: id }).then(function (res) {
      ...
      .on('click.ndfcact', '.btn-row-delete', function () {
        var id = this.getAttribute('data-id');
        function doDelete() {
          postJSON('/api/ndf-cockpit/row/delete', { date: currentDate(), id: id }).then(function (res) {
