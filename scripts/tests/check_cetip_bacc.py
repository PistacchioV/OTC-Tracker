...
try:
    ...
    marcados = sorted(k for k, v in CE._CETIP_BEHAVIOUR.items() if v.get('attach_bacc'))
    ...
    for k in marcados:
        cfg = CE._CETIP_BEHAVIOUR[k].get('bacc') or {}
        check('%-32s tem o par de colunas' % k,
              bool(cfg.get('parte')) and bool(cfg.get('contraparte')), True)
    check('as tres contas do intragrupo', sorted(CE._CETIP_BACC_ACCOUNTS),
          ['00041007', '73760009', '85398005'])
    check('o DFLUXO nao vai para mais ninguem',
          any(CE._CETIP_BEHAVIOUR['SWAP Flow (DFLUXO_SWAP)'].get(k)
              for k in ('attach_sales_support', 'attach_cem_latam')), False)

    print('\n== 2. conta comparavel: pontuacao fora, zero a esquerda de volta ==')
    check('00041.00-7 -> 00041007', CE._cetip_acct_key('00041.00-7'), '00041007')
    check('41007 (numero) -> 00041007', CE._cetip_acct_key('41007'), '00041007')
    check('73760009 fica', CE._cetip_acct_key('73760009'), '73760009')
    check('vazio nao vira conta', CE._cetip_acct_key('') in CE._CETIP_BACC_ACCOUNTS, False)

    print('\n== 3. posicao SWAP (sem cabecalho: Participante col D x Contraparte col H) ==')
    cfg = CE._CETIP_BEHAVIOUR['SWAP Position (DPOSICAO-SWAP)']['bacc']
    ...
    got = CE._cetip_bacc_copy(src, cfg, out)
    ...
    cfg = CE._CETIP_BEHAVIOUR['SWAP Flow (DFLUXO_SWAP)']['bacc']
    ...
    cfg = CE._CETIP_BEHAVIOUR['Option Position (OPC DPOSICAO)']['bacc']
    ...
    cfg = CE._CETIP_BEHAVIOUR['NDF Position (DPOSICAO-TER)']['bacc']
    ...
    check('devolve None (nao anexa)', CE._cetip_bacc_copy(src, ruim, out), None)
    check('arquivo vazio tambem devolve None',
          CE._cetip_bacc_copy(write(tmp, 'vazio.TXT', []), ruim, out), None)
    ...
    check('quatro chaves', sorted(CE._CETIP_RECIPIENT_KEYS),
          ['bacc_to', 'cem_to', 'hub_to', 'ss_to'])
    _real_load = CE._load_cetip_recipients
    CE._load_cetip_recipients = lambda: {'ss_to': 'ss@x.com', 'cem_to': 'cem@x.com',
                                        'bacc_to': 'bacc@x.com', 'hub_to': 'hub@x.com'}
    try:
        rec, mudou = CE._cetip_merge_recipients({'ss_to': 'novo@x.com'})
    ...
    SRC = read('apps/pages/features/cetip/engine.py')
    ...
    got = CE._cetip_txt_copy(src, out)
    check('nome do original + .txt',
    ...
    check('origem inexistente devolve None, nao explode',
          CE._cetip_txt_copy(os.path.join(tmp, 'nao-existe.CETIP21'), out), None)

    # Os QUATRO arquivos do HUB, e so eles.
    hub = sorted(k for k, v in CE._CETIP_BEHAVIOUR.items() if v.get('attach_hub'))
    ...
    ter = CE._CETIP_BEHAVIOUR['NDF Position (DPOSICAO-TER)']
    ...
    check('nome de anexo: .txt e acrescentado uma vez so',
          [CE._cetip_txt_name('73760_260817_DPOSICAO-TER.TER'),
           CE._cetip_txt_name('CETIP21_260817_DPOSICAOESTRATEGIA_MID.txt'),
           CE._cetip_txt_name('X.TXT')],
    ...
    check('o tipo novo nao vira JSON nem recorte',
          [k for k in CE._CETIP_BEHAVIOUR['SWAP (Strategy)']], ['attach_hub'])
    ...
    check('o parentetico e o nome do arquivo',
          [CE._cetip_paren_key('NDF Position (DPOSICAO-TER)'),
           CE._cetip_paren_key('SWAP (Strategy)'),
           CE._cetip_paren_key('sem parenteses')],
    ...
    parens = [CE._cetip_paren_key(k) for k in CE._CETIP_BEHAVIOUR]
    check('nenhum parentetico repetido entre os comportamentos',
          len(parens), len(set(parens)))
    exato = CE._cetip_behaviour_for('NDF Position (DPOSICAO-TER)')
    ...
    for antigo in ('Term Position (DPOSICAO-TER)', 'Posicao de Termo (DPOSICAO-TER)'):
        check('%r resolve IGUAL pelo parentetico' % antigo,
              CE._cetip_behaviour_for(antigo), exato)
    ...
    check('o Movement de termo tambem atende pelo nome antigo',
          CE._cetip_behaviour_for('Term Movement (DMOVIMENTO C21)'),
          CE._cetip_behaviour_for('NDF Movement (DMOVIMENTO C21)'))
    check('rotulo desconhecido devolve vazio, e nao explode',
          CE._cetip_behaviour_for('Coisa Nova (NAOEXISTE)'), {})
    check('rotulo vazio idem', CE._cetip_behaviour_for(''), {})
