...
| `check_swap_advice.py` | Settlement Advice de Swap: o universo **compartilhado** com o Trade Level (`_ops_swap_settling`, uma implementação só), a Data de Operação do **trade** (termo antes de início — errar sobe a alíquota do cliente), o IR encolhendo o líquido nos dois sinais, branco ≠ 0%, a página **ligada** (rota, endpoint, `data-api`, sidenav sem âncora morta), o **indexador** saindo da posição com o VCP resolvido pelo `Nome Tipo/Classe` da perna certa, e o **aviso impresso** (colunas a partir de Número de Contrato, R$ em BR, assunto e coluna de prêmio) | `_swadv_collect`/`_swadv_email_rows`/`_swadv_indexador`/`_ops_swap_settling`, `otc_emails.build_swap_settlement_emails` ou `other-products-swap-settlement-advice.html` (§184/§187) |
