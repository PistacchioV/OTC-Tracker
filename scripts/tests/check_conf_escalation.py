...
sem dar erro nenhum são as que este script prende:
  ...
  1. **o grupo de Front Office** — o produto casa pelo TIPO DE CONFIRMAÇÃO
     ...
     sem erro no log;
  2. **quem não casa não some calado** — Pending FO fora dos três grupos vai
...
Mais o contrato dos e-mails: os quatro assuntos por extenso (é o que a mesa
...
check('MO', R._CE_SUBJECT_MO, 'Confirmações Pendentes de Validação - MO')
subj = {g['id']: g['subject'] for g in R._CE_FO_GROUPS}
check('FO · CEM Swap', subj.get('cem-swap'),
      'Confirmações Pendentes de Validação - FO - CEM Swap')
check('FO · EDG Swap', subj.get('edg-swap'),
      'Confirmações Pendentes de Validação - FO - EDG Swap')
check('FO · EDG Option', subj.get('edg-option'),
      'Confirmações Pendentes de Validação - FO - EDG Option')
# SWAP e SWAP CORPORATE da EDG têm o MESMO assunto: são um e-mail só. Dois
# grupos com o mesmo título chegariam como duas mensagens iguais.
check('   SWAP e SWAP CORPORATE da EDG são UM grupo',
      len({g['subject'] for g in R._CE_FO_GROUPS}), 3)
...
check('SWAP CORPORATE × EDG', grupo('SWAP CORPORATE', 'EDG'), 'edg-swap')
...
check('   e a caixa/acento não decidem nada', grupo(' swap corporate ', 'edg'), 'edg-swap')
...
ROWS = [
    ...
    linha(T_FOLGA, M.PENDING_FO, 'CLIENTE FO EDG', 'SWAP CORPORATE', 'EDG', 'T6'),
    linha(T_FOLGA, M.PENDING_FO, 'CLIENTE FO OPT', 'OPTION', 'EDG', 'T7'),
    # Pending FO fora dos três grupos: NÃO some calado.
    linha(T_FOLGA, M.PENDING_FO, 'CLIENTE SEM GRUPO', 'NDF COMM', 'CEM', 'T8'),
    # Estados que não são de mesa nenhuma, e o fim da esteira.
...
try:
    ...
    mo, grupos, esc, sem_grupo = R._ce_snapshot(HOJE)
    ...
    check('EDG Swap leva o SWAP CORPORATE',
          [r['trade_id'] for r in por_id['edg-swap']['rows']], ['T6'])
    ...
    try:
        ...
        R._load_ce_recipients = lambda: {'sales_to': 'sales@x.com',
                                         'sales_escalation': 'boss@x.com',
                                         'fo_to': 'fo@x.com'}

        del enviados[:]
        out = R._ce_run('routine', datetime(2026, 8, 12, 17, 0))
        check('a rotina manda MO + os três grupos de FO',
              [e['subject'] for e in enviados],
              [R._CE_SUBJECT_MO, subj['cem-swap'], subj['edg-swap'], subj['edg-option']])
        check('   o MO vai para o Sales Support', enviados[0]['to'], ['sales@x.com'])
        check('   e o FO para o Front Office', enviados[1]['to'], ['fo@x.com'])
        ...
        del enviados[:]
        R._ce_run('fo-edg-swap', datetime(2026, 8, 12, 17, 0))
        check('o Run individual manda SÓ o e-mail daquele item',
              [e['subject'] for e in enviados], [subj['edg-swap']])
        del enviados[:]
        ...
        M.load_all = lambda: [dict(r) for r in ROWS]
        R._load_ce_recipients = lambda: {'sales_to': '', 'sales_escalation': '',
                                         'fo_to': ''}
        del enviados[:]
        out = R._ce_run('routine', datetime(2026, 8, 12, 17, 0))
