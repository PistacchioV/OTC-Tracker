...
- **Os JSON** (cache dos arquivos-dia, cadastros do /mapping, tickets,
  ...
  **A quebra dos bancos é por PRODUTO, um DB por arquivo-dia e um DB por JSON
  ...
  - **o escopo de `cache/` é um CAMINHO** (`new deals/NDF`), não só a rotina de
    primeiro nível: as duas rotinas grandes se repartem por dentro — uma fatia
    por pasta de produto —, porque como bloco único elas eram o gargalo da carga
    (as outras quatro terminam em minutos e três pessoas esperavam a maior). O
    `99_outros` poda por CAMINHO, e é isso que faz a rede de segurança valer nos
    DOIS níveis: uma rotina nova (`cache/equity`) e uma pasta nova dentro de uma
    já coberta (`cache/new deals/Equity`) caem os dois nele. Podando por
