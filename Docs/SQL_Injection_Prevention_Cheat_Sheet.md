<!--
  Documento EXTERNO, incorporado ao projeto sem alteração de conteúdo.

  Fonte:   https://github.com/nokia/OWASP-CheatSheetSeries/blob/master/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.md
  ...
  Só este comentário de procedência foi adicionado. Para atualizar, baixe o
  arquivo novamente da fonte e reponha este bloco no topo — não edite o texto
  do OWASP à mão, senão a próxima atualização vira um diff impossível de ler.

  Como isso se aplica ao OTC Tracker: ver a seção "SQL injection" no CLAUDE.md.
-->
